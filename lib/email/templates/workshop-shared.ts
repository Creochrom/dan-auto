...
export function renderWorkshopPhoneBannerText(
  ...
  const phone = formatWorkshopField(customerPhone);
...
export function renderWorkshopContactText(fields: WorkshopContactFields): string[] {
  return [
    ...
    `Phone number:           ${formatWorkshopField(fields.customerPhone)}`,
...
export function renderWorkshopPhoneBannerHtml(
  ...
  return `<div style="background:#1a3a1a;border:2px solid #22c55e;border-radius:10px;padding:16px 18px;margin:0 0 20px">
    ...
    <p style="margin:0;font-size:28px;font-weight:700;line-height:1.2">
      <a href="tel:${escapeHtml(tel)}" style="color:#4ade80;text-decoration:none">${escapeHtml(phone)}</a>
    </p>
...
export function renderWorkshopContactHtml(fields: WorkshopContactFields): string {
  const phone = formatWorkshopField(fields.customerPhone);
  const tel = safePhoneHref(phone);
  ...
  <table style="width:100%;border-collapse:collapse;margin:0 0 20px">
    ...
    <tr><td style="padding:5px 12px 5px 0;color:#71717a">Phone</td><td style="padding:5px 0"><a href="tel:${escapeHtml(tel)}" style="color:#4ade80;font-size:16px;font-weight:600">${escapeHtml(phone)}</a></td></tr>
    <tr><td style="padding:5px 12px 5px 0;color:#71717a">Email</td><td style="padding:5px 0">${fields.customerEmail?.trim() ? `<a href="mailto:${escapeHtml(fields.customerEmail.trim())}" style="color:#22d3ee">${escapeHtml(fields.customerEmail.trim())}</a>` : NOT_PROVIDED}</td></tr>
